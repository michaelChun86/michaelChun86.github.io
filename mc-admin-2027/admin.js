...
(function () {
  ...
  /* 유입 경로 도넛/표에 쓰는 색.
     예전에는 사이트 색인 보라·청록 계열로만 다섯 개를 뽑았더니 1번과 4번,
     2번과 5번이 거의 같은 색으로 보여 어느 조각이 어느 경로인지 알기 어려웠다.

     그래서 색상환을 정확히 72도씩 나눈 다섯 색으로 바꿨다(무지개처럼).
     기준점은 사이트 대표색인 보라(267도)라 분위기는 그대로 이어진다.
       · 색상각 간격     72도 (최대치 — 다섯 색으로 더 벌릴 수 없다)
       · 패널 대비       모두 8:1 이상
       · 휘도 폭         1.43배. 노랑은 본래 밝은 색이라 억지로 어둡게 맞추면
                        올리브색이 되므로 완전히 같게 두지 않고 폭만 좁혔다.
     ★ 나열 순서도 의미가 있다. 도넛은 조각이, 표는 행이 서로 맞닿으므로
       "이웃한 색"이 비슷하면 제일 헷갈린다. 다섯 색의 모든 순서를 견줘
       이웃 간 색 차이가 가장 큰 배열을 골랐다(최소 거리 97 → 239).
       색을 더하거나 뺄 때는 이 순서도 다시 살펴볼 것. */
  var PALETTE = ["#c798ff", "#ecc906", "#21c0f6", "#ff88b1", "#25d02d"];
